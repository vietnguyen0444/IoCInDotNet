...
namespace OH.Utilities
{
    public static class Pbkdf2HashPassword
    {
        ...
        /// <returns></returns>
        public static bool ValidatePassword(string password, string token , string salt, string correctHash)
        {
            password += ":" + token;

            byte[] _password = (new UTF8Encoding()).GetBytes(password);
            byte[] _salt = (new UTF8Encoding()).GetBytes(salt);
            byte[] hash = Convert.FromBase64String(correctHash);

            byte[] testHash = PBKDF2(_password, _salt, PBKDF2_ITERATIONS, hash.Length);
            var res = SlowEquals(hash, testHash);
